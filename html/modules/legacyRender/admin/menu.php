<?php

$adminmenu[10]['title']=_MI_XCORERENDER_ADMENU_TEMPLATE_MANAGE;
$adminmenu[10]['link']="admin/index.php?action=TplsetList";
$adminmenu[10]['show']=true;

$adminmenu[20]['title']=_MI_XCORERENDER_ADMENU_TPLFILE_MANAGE;
$adminmenu[20]['link']="admin/index.php?action=TplfileList";
$adminmenu[20]['show']=true;

$adminmenu[30]['title']=_MI_XCORERENDER_ADMENU_BANNER_MANAGE;
$adminmenu[30]['link']="admin/index.php?action=BannerList";
$adminmenu[30]['show']=true;

$adminmenu[40]['title']=_MI_XCORERENDER_ADMENU_HTACCESS_VIEW;
$adminmenu[40]['link']="admin/index.php?action=HtaccessView";
$adminmenu[40]['show']=true;

?>
