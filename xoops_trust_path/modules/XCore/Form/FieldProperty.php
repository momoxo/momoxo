...
use XCore\Form\DependClassFactory;
...
class FieldProperty
{
    ...
    public function setDependsByArray($dependsArr)
    {
        foreach ($dependsArr as $dependName) {
            $instance =& DependClassFactory::factoryClass($dependName);
            if ($instance !== null) {
     ...
     *   These additional parameters are used by Utils::formatString().
     * \code
    ...
    /**
     * Gets the error message rendered by Utils::formaString().
     *
     *   Gets the error message registered at addMessage(). If the message setting has some
     *   arguments, messages are rendered by Utils::formatString().
     * \code
     *   $field->addMessage('required', "{0:ucFirst} is requred.", "name");
     *
    ...
    public function renderMessage($name)
    {
        ...
        if ( isset($this->mMessages[$name]['args']) ) {
            // Use an unity method.
            $message = Utils::formatString($message, $this->mMessages[$name]['args']);
        }
