...
function make_data(&$dbm, &$cm, $adminname, $adminpass, $adminmail, $language, $gruops, $timezone){
	...
	$dbm->insert('config', " VALUES (7, 0, 1, 'theme_set', '_MD_AM_DTHEME', 'xcore_default', '_MD_AM_DTHEMEDSC', 'theme', 'other', 12)");
...
//	$dbm->insert('config', " VALUES (46, 0, 2, 'avatar_minposts', '_MD_AM_AVATARMP', '0', '_MD_AM_AVATARMPDSC', 'textbox', 'int', 15)");
	...
	$dbm->insert('config', " VALUES (72, 0, 1, 'theme_set_allowed', '_MD_AM_THEMEOK', '".serialize(array('xcore_default'))."', '_MD_AM_THEMEOKDSC', 'theme_multi', 'array', 13)");
	...
	$temp = md5($adminpass);
	$regdate = time();
	$dbm->insert('users', " VALUES (1,'','".addslashes($adminname)."','".addslashes($adminmail)."','".XOOPS_URL."/','blank.gif','".$regdate."','','','',1,'','','','','".$temp."',0,0,7,5,'xcore_default','".$time_diff."',".time().",'thread',0,1,0,'','','',0)");
